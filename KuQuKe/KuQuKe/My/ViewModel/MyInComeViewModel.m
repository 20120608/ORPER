...
#import "MyInComeViewModel.h"
#import "TaskTableViewCell.h"//cell的样式

@interface MyInComeViewModel()
//数据源
...
@implementation MyInComeViewModel
...
- (void)setupBind {
	...
	_requestVideoListCommand = [[RACCommand alloc] initWithSignalBlock:^RACSignal * _Nonnull(id  _Nullable input) {
		return [RACSignal createSignal:^RACDisposable * _Nullable(id<RACSubscriber>  _Nonnull subscriber) {
			...
			[KuQuKeNetWorkManager GET_myAccountLogParams:params View:self.currentVC.view success:^(RequestStatusModel *reqsModel, NSDictionary *dataDic) {
	...
	[_requestVideoListCommand.executionSignals.switchToLatest subscribeNext:^(id  _Nullable x) {
		//    NSLog(@"请求完成后的回调");
		[[NSNotificationCenter defaultCenter] postNotificationName: NotificationName_MyInComeViewController object:x];
	}];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	cell.myInDrowModel = _models[indexPath.row];
	return cell;
}
